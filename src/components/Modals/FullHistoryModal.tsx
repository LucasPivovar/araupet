      ...
      <div className="w-full max-w-sm bg-white rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200 flex flex-col max-h-[85vh]">
        ...
        <div className="bg-gradient-to-r from-[#008779] to-[#006e63] p-5 text-white flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base">Histórico Clínico & Vacinal</h3>
              <p className="text-xs text-teal-100">{pet.name} • {pet.breed}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-4 overflow-y-auto custom-scrollbar flex-1">
          ...
          <div className="p-3 rounded-2xl bg-teal-50 border border-teal-200 flex items-center justify-between text-xs">
            <div>
              <p className="font-bold text-slate-800">Microchip Oficial</p>
              <p className="font-mono text-[11px] text-[#008779]">982 000 193 847 210</p>
            </div>
            <span className="px-2 py-1 rounded-md bg-[#008779] text-white font-bold text-[10px]">
              Ativo
            </span>
          </div>
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-slate-700">Registros em Prontuário</h4>
            {records.map((r, i) => (
              <div key={i} className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-1.5 text-xs">
                ...
                <div className="text-[11px] text-slate-500 space-y-0.5">
                  <p>📅 <strong>Aplicação:</strong> {r.date} • <strong>Validade:</strong> {r.validity}</p>
                  <p>📍 {r.location}</p>
                  <p>👨‍⚕️ {r.vet}</p>
          ...
          <button
            ...
            className="w-full py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-colors shadow-xs"
          >
